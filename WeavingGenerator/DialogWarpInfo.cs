...
{
    public partial class DialogWarpInfo : DevExpress.XtraEditors.XtraForm
    {
        MainForm mainForm;
        ProjectData wData;
        //int SELECTED_IDX = -1;
        DataTable tb = new DataTable();
        //List<WInfo> listWarpInfo;
        List<Yarn> yarnList;

        string COLUMN_CHECK = "선택";
        string COLUMN_IDX = "번호";
        //string COLUMN_WARPNAME = "경사명";
        ...
        public DialogWarpInfo(MainForm main, ProjectData data)
        {
            this.mainForm = main;
            this.wData = data;
        ...
        private void DialogWarpInfo_Load(object sender, EventArgs e)
        {
            ...
            //tb.Columns.Add(COLUMN_WARPNAME);
        ...
        private void colorChangedClick(object sender, EventArgs e)
        {
            ...
            if(oldColor != newColor)
            {
                ...
                WInfoTemp warpInfo = GetWInfoTemp(gridView1.FocusedRowHandle);
                warpInfo.SetColor(colorEdit.Color);
        ...
        private List<WInfoTemp> ToWInfoTempList(Warp warp)
        {
            ...
            List<WInfo> wInfoList = warp.GetWInfoList();
            if (wInfoList == null)
            {
                return list;
            }
        ...
        private void btn_OK_Click(object sender, EventArgs e)
        {
            ...
            List<WArray> arrList = wData.Warp.GetWArrayList();
            List<WRepeat> repList = wData.Warp.GetWRepeatList();
            List<WInfo> wInfoList = new List<WInfo>();

            if (wInfoTempList.Count <= 0)
            {
                XtraMessageBox.Show("경사를 1개 이상 설정해야 합니다.");
                return;
            }
            //Trace.WriteLine("\n===========================================");
            for (int i = 0; i < wInfoTempList.Count; i++)
            ...
            for (int i = 0; i < wInfoTempList.Count; i++)
            {
                ...
                {
                    int newIdx = 0;
                    List<WArray> list2 = wData.Warp.GetWArrayList();
            ...
            wData.Warp.SetWInfoList(wInfoList);
            wData.Warp.SetWArrayList(arrList);
            wData.Warp.SetWRepeatList(repList);
            if(isModified == true)
            {
                // 업데이트
                mainForm.UpdateProjectData();
        ...
        private void simpleButton_Color_Click(object sender, EventArgs e)
        {
            ...
            if(nCnt <= 0)
            {
                XtraMessageBox.Show("경사를 1개 이상 선택해주세요...");
                return;
            }
    ...
    public class WInfoTemp
    {
        private int idx;
        private string name;
        private string hexColor;
        private int idxYarn;
        private string nameYarn;
        ...
        public override string ToString()
        {
            ...
            sb.Append("idx : " + idx + "\r\n");
            sb.Append("name : " + name + "\r\n");
            sb.Append("hexColor : " + hexColor + "\r\n");
            sb.Append("idxYarn : " + idxYarn + "\r\n");
            sb.Append("nameYarn : " + nameYarn + "\r\n");

            return sb.ToString();
        }
    }
    class CustomColorPickEdit : RepositoryItemColorPickEdit
    {
        protected override string GetColorDisplayText(Color editValue)
        {
            if (editValue.IsNamedColor)
            {
                return editValue.Name;
            }
            //return $"{editValue.A:X2}{editValue.R:X2}{editValue.G:X2}{editValue.B:X2}";
            return $"{editValue.R:X2}{editValue.G:X2}{editValue.B:X2}";
        }
    }
}
