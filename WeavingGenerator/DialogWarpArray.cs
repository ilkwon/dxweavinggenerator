...
{
    public partial class DialogWarpArray : DevExpress.XtraEditors.XtraForm
    {
        // 경위사 배열 자료 구조
        ...
        public DialogWarpArray(MainForm main, ProjectData data)
        {
            this.mainForm = main;
            this.data = data;
            InitializeComponent();
        ...
        private void DialogWarpArray_Load(object sender, EventArgs e)
        {
            if(data == null)
            {
                return;
            ...
            arrayTempList = ToArrayTempList(data.Warp);
            ...
            repeatTempList = ToRepeatTempData(data.Warp);
            ...
            // 경사 배열
            ///////////////////////////////////////////////////////////////////
            List<WArray> list = warp.GetWArrayList();
            for (int i = 0; i < list.Count; i++)
            {
                WArray arr = list[i];
                int idx = arr.Idx;
                int cnt = arr.Count;
                WInfo info = warp.GetWInfo(idx);
        ...
        private void btn_OK_Click(object sender, EventArgs e)
        {
            ...
            data.Warp.SetWArrayList(arrInfoList);
            data.Warp.SetWRepeatList(repeatList);
        ...
        private List<ArrayTempData> ToArrayTempList(Warp warp)
        {
            ...
            List<WArray> arrayInfo = warp.GetWArrayList();
            ...
            for (int i = 0; i < wInfoList.Count; i++)
            {
                ...
                ArrayTempData arr = new ArrayTempData("경사"+(i+1), idx, hexColor, valueList);
        ...
        private List<WArray> ToWArray(List<ArrayTempData> arrayTemp)
        {
            ...
            for (int i = 0; i < arrayTemp.Count; i++)
            {
                ...
                for (int j = 0; j < n; j++)
                {
                    ...
                    {
                        WArray warpArrayInfo = list[j];
                        warpArrayInfo.Idx = warpIdx;
                        warpArrayInfo.Count = Convert.ToInt32(value);
                    }
                }
            }
            return list;
